...
// functions
job_t *add_job(pid_t pid, int status);
...
void check_jobs() {
    job_t *p;
    for (p = jobs; p; p = p->next) {
        if (waitpid(p->pid, &status, WNOHANG | WUNTRACED)) {
            if (WIFSTOPPED(status)) {
                p->status = JOB_STOPPED;
                printf("\n[%d]  Stopped\t\t\t%s\n", p->pid, p->cmd);
